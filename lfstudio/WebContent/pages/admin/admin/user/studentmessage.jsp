...
<body>
	...
	<section>
				...
				<div class="col-md-12 column">
					<div class="row clearfix">
						<div class="col-md-8 column">
							...
							<form:form role="form" action="<%= path %>" method="post"
								modelAttribute="person" onsubmit="return checkform();">
								<input type="hidden" id="headimgsrc" name="headimg"
									value="${person.headimg }" />
								<div class="row clearfix">
									...
									<div class="col-md-6 column">
										...
										<div class="form-group">
											<label>性别:</label><br /> <label class="radio-inline"><form:radiobutton
													path="sex" value="0" />男</label> <label class="radio-inline"><form:radiobutton
													path="sex" value="1" />女</label>
										</div>
										<div class="form-group">
											<label>出生日期:</label> <input type="date" class="form-control dateinput"
												value="${birthday }" name="date" />
										</div>
									</div>
								</div>
								<div class="form-group">
						...
						<div class="col-md-4 column">
							<div class="head">
								<div class="headimgbox">
									<img id="headimg" alt="请上传头像"
										src="<%= request.getContextPath() %>${person.headimg}"
										class="img-thumbnail" />
								</div>
								<br />
		...
		<div class="hidden_form">
			<input type="text" id="hidden_form_name" value="${person.name }" />
			<input type="text" id="hidden_form_birthpath"  value="${person.birthpath }" />
			<input type="text" id="hidden_form_nation" value="${person.nation }" />
			<input type="text" id="hidden_form_college"  value="${person.college }" />
			<input type="text" id="hidden_form_major" value="${person.major }" />
			<input type="text" id="hidden_form_majorclass"  value="${person.majorclass }" />
			<input type="text" id="hidden_form_sex" value="${person.sex }" />
			<input type="text" id="hidden_form_birthday"  value="${birthday }" />
			<input type="text" id="hidden_form_dormitory" value="${person.dormitory }" />
			<input type="text" id="hidden_form_room"  value="${person.room }" />
			<input type="text" id="hidden_form_homepath"  value="${person.homepath }" />
			<input type="text" id="hidden_form_headimg"  value="${person.headimg }" />
		</div>
	</section>
	<script type="text/javascript">
		...
		function checkform(){
			...
			var flag = false;
			if($("#hidden_form_name").val()!=$("#name").val()){
				flag = true;
			}
			if($("#hidden_form_birthpath").val()!=$("#birthpath").val()){
				flag = true;
			}
			if($("#hidden_form_nation").val()!=$("#nation").val()){
				flag = true;
			}
			if($("#hidden_form_college").val()!=$("#college").val()){
				flag = true;
			}
			if($("#hidden_form_major").val()!=$("#major").val()){
				flag = true;
			}
			if($("#hidden_form_majorclass").val()!=$("#majorclass").val()){
				flag = true;
			}
			if($("#hidden_form_birthday").val()!=$(".dateinput").val()){
				flag = true;
			}  
			if($("#hidden_form_dormitory").val()!=$("#dormitory").val()){
				flag = true;
			}
			if($("#hidden_form_room").val()!=$("#room").val()){
				flag = true;
			}
			if($("#hidden_form_homepath").val()!=$("#homepath").val()){
				flag = true;
			}
			if($("#hidden_form_headimg").val()!=$("#headimgsrc").val()){
				flag = true;
			}
			if($("#sex1").is(":checked")){
				if($("#sex1").val() != $("#hidden_form_sex").val()){
					flag = true;
				}
			}else{
				if($("#sex2").val() != $("#hidden_form_sex").val()){
					flag = true;
				}
			}
			if(flag){

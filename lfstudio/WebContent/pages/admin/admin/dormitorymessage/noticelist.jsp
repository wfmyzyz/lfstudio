			...
			<div class="row clearfix">
				<div class="col-md-12 column">
					...
					<table class="table table-bordered table-hover">
					    ...
					    <tbody>
					    	<c:forEach items="${noticelists }" var="noticelist">
						    	<tr>
							        <td>${noticelist.id }</td>
							        <td><span  class="title"><a class="optiona" href="<%= request.getContextPath() %>/admin/dormitortnotice.shtml?id=${noticelist.id }">${noticelist.text }</a></span></td>
							        <td>${noticelist.date }</td>
							        <td><a class="option delete" data-id="${noticelist.id }" href="javascript:void(0);">删除</a></td>
						      	</tr>
					    	</c:forEach>
					    </tbody>
					</table>
	...
	<script type="text/javascript">
		(function(){
			...
			$(".delete").click(function(){
				if(confirm("是否删除编号为"+$(this).attr("data-id")+"的公告？")){
					window.location.href = "<%= request.getContextPath() %>/admin/deletenotice.shtml?id="+$(this).attr("data-id");
				}
			})
		}(jQuery))
	</script>

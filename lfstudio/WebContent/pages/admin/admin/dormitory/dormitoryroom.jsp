		...
		<div class="container">
			<div class="row clearfix">
				<div class="col-md-12 column">
					 <a href="<%= request.getContextPath()+"/admin/optiondormitoryroom.shtml?act=insert&pid="+request.getParameter("pid") %>" class="btn btn-default btn-block btn-success">新增宿舍房间</a>
				</div>
			</div>
			<div class="row clearfix dormitory">
				...
				<c:forEach items="${roomlist }" var="room">
					...
					<div class="col-md-3 column dormitory-outbox">
						<div class="dormitory-box"><h3>${room.floor}-${room.name }</h3><i class="fa fa-close remove" domitoryname="${room.name }" domitoryid="${room.id }"></i></div>
					</div> 
				</c:forEach>
			</div>
		</div>

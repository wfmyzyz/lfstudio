...
<body>
					...
					<div class="row clearfix">
						...
						<% String path = request.getContextPath()+"/admin/optionelectric.shtml"; %>
							<form role="form" action="<%= path %>" onsubmit="return checkform();">
								...
								<div class="form-group">
									 <label>充值金额<span>（每度电0.6元）</span></label>
									 <input type="text" class="form-control" id="money" name="money" />
								</div>
								<input class="btn btn-default btn-info rounded" type="submit" value="充值" />
							</form>
						</div>
						<div class="col-md-6 column">
						</div>
	...
	<script type="text/javascript">
		...
		function checkform(){
			...
			if(confirm("是否为"+$("#dormitory option:selected").text()+"的"+$("#room option:selected").text()+"宿舍充值"+$("#money").val()+"元?")){
				return true;
			}
			return false;
		}
